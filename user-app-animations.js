  ...
  if(period !== 'all') {
    ...
    filteredData = filteredData.filter(user => {
      ...
      return user.transactions.some(transaction => {
        // Use timestamp if date doesn't exist, fallback to current time if neither exists
        const transactionTime = transaction.date || transaction.timestamp || Date.now();
        const entryDate = new Date(transactionTime);
...
function exportAdminData() {
  try {
    ...
    // Professional CSV Header with Serbian characters
    csvContent.push('=== PREGLED POTROŠNJE PO KORISNICIMA ===');
    csvContent.push('Firma: ' + currentFirma);
    csvContent.push('Datum izvoz: ' + currentDate);
    csvContent.push('Konverzija: 1 kredit = 100 RSD');
    csvContent.push('');
    
    // Summary section with proper formatting
    csvContent.push('=== UKUPNI STATISTIKA ===');
    csvContent.push('Ukupno zaposlenih: ' + document.getElementById('adminTotalEmployees').textContent);
    csvContent.push('Ukupno kredita: ' + document.getElementById('adminTotalTokens').textContent);
    csvContent.push('Ukupno RSD: ' + document.getElementById('adminTotalRSD').textContent);
    csvContent.push('Ukupno pranja: ' + document.getElementById('adminTotalWashes').textContent);
    csvContent.push('');
    
    // User summary table with proper headers and spacing
    csvContent.push('=== DETALJNI PREGLED PO KORISNICIMA ===');
    csvContent.push('Korisnik,UID,Ukupno Kredita,Ukupno RSD,Poslednji Box,Poslednje Pranje');
    ...
    // Create CSV rows for user summary with proper formatting
    Object.values(userGroups).forEach(user => {
      const lastWashDate = user.lastWash ? new Date(user.lastWash).toLocaleDateString('sr-RS') : 'N/A';
      const lastWashTime = user.lastWash ? new Date(user.lastWash).toLocaleTimeString('sr-RS', {hour: '2-digit', minute: '2-digit'}) : '';
      const fullLastWash = lastWashDate !== 'N/A' ? `${lastWashDate} ${lastWashTime}` : 'N/A';
      
      // Format with proper quotes and escaping for professional CSV
      csvContent.push(`"${user.name}","${user.uid}",${user.totalCredits},"${user.totalRSD.toLocaleString()} RSD","${user.lastBox}","${fullLastWash}"`);
    });
    
    // Detailed transaction history section with professional formatting
    csvContent.push('');
    csvContent.push('=== DETALJNA ISTORIJA SVIH TRANSAKCIJA ===');
    csvContent.push('Korisnik,UID,Datum,Vreme,Box,Krediti,RSD,Timestamp');
    ...
    allTransactions.forEach(t => {
      // Use timestamp if date doesn't exist, fallback to current time if neither exists
      const transactionTime = t.date || t.timestamp || Date.now();
      const date = new Date(transactionTime).toLocaleDateString('sr-RS');
      const time = new Date(transactionTime).toLocaleTimeString('sr-RS', {hour: '2-digit', minute: '2-digit'});
      const box = t.box || 'N/A';
      const rsd = t.rsd || (t.amount * 100);
      
      // Professional CSV formatting with proper quotes and escaping
      csvContent.push(`"${t.name}","${t.uid}","${date}","${time}","${box}",${t.amount},"${rsd.toLocaleString()} RSD","${t.timestamp}"`);
    });
    
    // Download CSV with proper encoding for Serbian characters
    const csvString = csvContent.join('\n');
    const blob = new Blob(['\ufeff' + csvString], { type: 'text/csv;charset=utf-8;' }); // BOM for Excel compatibility
    const link = document.createElement('a');
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `admin_professional_report_${currentFirma}_${currentDate}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    showStatus("userStatus", "📊 Profesionalan CSV fajl sa detaljnim podacima je preuzet!", "success");
  } catch (error) {
    console.error('Error exporting CSV:', error);
    showStatus("userStatus", "❌ Greška pri izvozu CSV fajla", "error");
  }
}
